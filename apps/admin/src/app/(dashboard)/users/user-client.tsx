...
import { Card, CardContent, ConfirmDialog, EmptyState, PageHeader, Pagination } from "@ledgr/ui";
...
export default function AdminUsersPageClient({ initialUsers, initialTrans }: Props) {
    const [users, setUsers] = useState<Profile[]>(initialUsers);
    const [search, setSearch] = useState("");
    const [statusFilter, setStatusFilter] = useState<"all" | UserStatus>("all");
    const [selectedUser, setSelectedUser] = useState<Profile | null>(null);
    const [confirmAction, setConfirmAction] = useState<{ user: Profile; action: UserStatus } | null>(null);
    ...
    const handleStatusChange = async (userId: string, newStatus: UserStatus) => {
        try {
            ...
            setUsers((prev) =>
                prev.map((u) =>
                    u.id === userId ? { ...u, status: newStatus === 'active' ? 'suspended' : 'active' } : u
                )
            );
